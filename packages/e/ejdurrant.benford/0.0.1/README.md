# EJDurrant.Benford
Measure and display the conformance, or otherwise, of a column of numbers with Benford's Law.
...
For detailed information on Benford's Law, a book is available:
...
## Power Pivot
The Average Deviation function can be adapted to work in PowerPivot, which will almost always be more useful than Power BI in an accounting context. You will have to change the line that generates the initial list of numbers, by removing `GENERATESERIES ( 10, 99, 1 ), "First Two Digits", [Value]` and replacing it with `CALENDAR ( 10, 99 ), "@Two Digits", INT ( [Date] )`. This does exactly the same thing, and works in PowerPivot.
